...
public class BackgroundGeolocation extends Plugin {
    ...
    // permission is actually granted can throw and crash the app straight
    // back to the home screen — but only the FIRST time, since every later
    // call already has permission granted before reaching this point at all.
    //
    // Fix: split the actual "start the watcher" logic into its own method,
    // and only call it once permission is confirmed. If permission still
    // needs to be requested, save the call and return — the async permission
    // callback (locationPermissionsCallback) is the ONLY thing that resumes
    // from there.
    ...
    private void startWatcherNow(final PluginCall call) {
        ...
        // Read the new options WorkPay's JS now passes into addWatcher().
        // All optional — if omitted, the native push simply won't run for
        // this watcher, same as before the patch.
        service.addWatcher(
            call.getCallbackId(),
    ...
    // ── WORKPAY NATIVE PUSH FIX ──────────────────────────────────────────
    // New plugin method: call this from index.html whenever Labour Activity
    // changes a clocked-in worker's geofence radius/site while the native
    // watcher is already running, so the next background fix uses the
    // current geofence rather than the one captured at punch-in.
    @PluginMethod()
    public void updateGeofence(PluginCall call) {
        String workerId = call.getString("workerId");
        if (workerId == null || service == null) {
            call.reject("Missing workerId or service not running.");
            return;
        }
