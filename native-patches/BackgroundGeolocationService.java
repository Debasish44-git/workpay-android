...
public class BackgroundGeolocationService extends Service {
    ...
    public class LocalBinder extends Binder {
        ...
        // v90 FIX: the JWT passed in at addWatcher() time expires (typically
        // ~1hr) — a shift can easily outlast that. index.html calls this on a
        // timer (see wpRefreshNativeAuthToken) with the freshly-refreshed
        // session token, same pattern as updateGeofence above but for auth
        // instead of the geofence. Without this, pushes would work at the
        // start of a long shift and silently go back to failing partway
        // through once the original token expires.
        void updateAuthToken(String workerId, String authToken) {
            for (Watcher watcher : watchers) {
                if (workerId.equals(watcher.workerId)) {
                    watcher.authToken = authToken;
                }
